...
import {
  getContext,
  getVideoContextsWithTranslations,
} from '../Redux/Context/selectors'
...
function renderWord(word, mainFont, smallerFont, rootVerb) {
    ...
    return (
      <div>
        ...
        <div style={{ fontSize: smallerFont, marginBottom: 5 }}>
          ...
          {tenseLabels[word.tense] || 'infinitive'}
        </div>
  ...
  return (
    <div>
      <div style={{ fontSize: smallerFont }}>
        {partsOfSpeech[word.partOfSpeech]}
      </div>
      <div style={{ fontSize: mainFont }}>
...
function renderWordPopup(
  ...
  if (!translation && possibleTranslations.length > 0) {
    return (
      ...
      >
        <div style={{ fontSize: smallerFont, paddingBottom: 10 }}>
  ...
  if (translation.wordId === 'NOT_A_WORD') {
    return (
      <div style={{ backgroundColor: 'white', padding: 10, borderRadius: 4 }}>
        <div style={{ fontSize: mainFont, marginBottom: 5 }}>
          Not a spanish word / Unknown
        </div>
      </div>
    )
  }
  ...
  foundWords
    ...
    .forEach((foundWord, index) => {
      ...
      const translation = translations.find(
        (t) => t.textStart === textStart && t.textEnd === textEnd
      )
      subtitles.push({
        id: `${textStart}_${textEnd}`,
...
function Video(props) {
  ...
  const videoContexts = useSelector(
    getVideoContextsWithTranslations(props.videoId)
  )
  ...
  const [currentContextId, setCurrentContextId] = useState(
    getContextIdWithTime(startingTime)
  )
  ...
  const currentContextIndex = videoContexts.findIndex(
  ...
  const setPopover = (translationId, isOpen) => {
    ...
    const [textStart, textEnd] = translationId
      .split('_')
      .map((n) => parseInt(n))
  ...
  function getContextIdWithTime(time) {
    const availableContexts = videoContexts.filter(
      (context) => context.startTime < time
    )
  ...
  const onFirstSubtitle = () => {
    const newContext = videoContexts.sort(
  ...
  return (
    ...
    >
      <InfoModal
        ...
        description={
          'Remember you can click on subtitles to find out the meaning of each word'
        }
      />
      <AccountNeededModal
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            position: 'absolute',
            bottom:
              isHovering || !isPlaying || isNewVideo ? (isMobile ? 60 : 50) : 3,
        ...
        >
          ...
          >
            {!currentContext && !isNewVideo && videoContexts.length > 0 && (
              <div style={{ cursor: 'pointer' }} onClick={onFirstSubtitle}>
                Click to fast-forward to the first subtitle
              </div>
            )}
                  ...
                  {subtitle.type === 'filler' ? (
                    ...
                    >
                      <span
                        style={
                          props.wordId &&
                          subtitle.translation?.flashcardWordId === props.wordId
            ...
            {currentContext &&
              currentContextIndex !== videoContexts.length - 1 && (
                <ArrowForwardIosIcon
                  style={
                    isMobile
                      ? { cursor: 'pointer', height: 15 }
