...
const subs = [
  ...
  },
  {
    ...
    endTime: 25,
    subtitle:
      'me volviste más dura las mujeres ya nome volviste más dura las mujeres ya nome volviste más dura las mujeres ya nome volviste más dura las mujeres ya nome volviste más dura las mujeres ya nome volviste más dura las mujeres ya nome volviste más dura las mujeres ya no',
  },
]
...
const Timeline = (props) => {
      ...
      if (isResizing) {
        if (resizingSide === 'LEFT') {
          ...
          const newTime = Math.min(
            Math.max(resizingInitialStartTime + s, min),
            max
          )
        ...
        if (resizingSide === 'RIGHT') {
          ...
          const max =
            nextSubs.length > 0 ? Math.min(...nextSubs) - 0.1 : duration
          const min = resized.startTime + 0.1

          const newTime = Math.min(
            Math.max(resizingInitialEndTime + s, min),
            max
          )
  ...
  const onDrop = (event) => {
    ...
    const conflicting = subtitles
      ...
      .find(
        (sub) =>
          (sub.startTime <= newStartTime && sub.endTime >= newStartTime) ||
      ...
      {subtitles.map(({ id, startTime, endTime, subtitle }) => {
        return (
          ...
          >
            <div
              className={'leftSide'}
              onMouseDown={(event) => onLeftMouseDown(id, event)}
            ></div>
            <div
              style={{
                height: '100%',
            ...
            <div
              className={'rightSide'}
              onMouseDown={(event) => onRightMouseDown(id, event)}
            ></div>
          </div>
        )
      })}
