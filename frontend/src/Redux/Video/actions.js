...
import {
  FETCH_MY_VIDEOS,
  FETCH_VIDEOS, SET_MY_VIDEOS_LOADING, SET_VIDEOS_LOADING, LIKE_VIDEO, LIKE_CLIP, SET_LIKED_VIDEOS_LOADING, FETCH_LIKED_VIDEOS
...
export const fetchAllVideos = (page) => (dispatch) => {
  ...
  fetch(`${API_URL}/getAllVideos?language=ESP&page=` + page * 20)
    ...
    .then((data) => {
      ...
      videos
        .map(parseVideo)
        .forEach(video => { videoMap[video.id] = video })
...
export const fetchMyVideos = (page) => (dispatch, getState) => {
  ...
  fetch(`${API_URL}/getAllVideos?language=ESP&page=${page * 20}&user_id=${userId}`)
    .then((res) => res.json())
    .then((data) => {
      const { videos } = data
      const areMoreVideos = videos.length === 20
...
export const fetchLikedVideos = (page) => (dispatch, getState) => {
  ...
  fetch(`${API_URL}/getLikedVideos?language=ESP&page=${page * 20}&user_id=${userId}`)
    .then((res) => res.json())
    .then((data) => {
      const { videos } = data
      const areMoreVideos = videos.length === 20
...
export const likeClip = (videoId, value, wordId, contextId, token) => (dispatch, getState) => {
