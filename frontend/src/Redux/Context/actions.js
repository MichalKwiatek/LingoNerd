...
export const addContexts =
  (videoId, language, subtitles, onSuccess, onFailure, token) =>
  (dispatch, getState) => {
...
export const createTranslations =
  (translations, words, onSuccess, onFailure, token) =>
  (dispatch, getState) => {
    dispatch({
...
export const updateSubtitle =
  (contextId, text, onSuccess) => (dispatch, getState) => {
...
export const addVideo =
  (id, title, url, language, onSuccess, onFailure, token) =>
  (dispatch, getState) => {
    const creationTimestamp = getCurrentDate().getTime()
...
export const fetchContexts =
  ...
  (dispatch) => {
    fetch(
      `${API_URL}/getVideos?language=ESP&wordId=${encodeURI(
        wordId
      )}&limit=${limit}`
    )
      .then((res) => res.json())
      .then((data) => {
        ...
        const contextsMap = {}
        const contexts = data.contexts.map((context) => parseContext(context))
        contexts.forEach((context) => {
          contextsMap[context.id] = context
        })
        ...
        translations.forEach((translation) => {
          if (contextTranslations.hasOwnProperty(translation.contextId)) {
            contextTranslations[translation.contextId] = [
              ...contextTranslations[translation.contextId],
              translation.id,
            ]
          } else {
          ...
          if (wordTranslations.hasOwnProperty(translation.flashcardWordId)) {
            wordTranslations[translation.flashcardWordId] = [
              ...wordTranslations[translation.flashcardWordId],
              translation.id,
            ]
          } else {
...
export const getContextTranslation =
  ({ textStart, textEnd, contextId, text, onPossibleTranslations }) =>
  (dispatch) => {
    fetch(`${API_URL}/get-context-translation`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        const { translation, words, possibleTranslations } = JSON.parse(
          data.body
        )
        ...
        if (translation) {
          ...
          onPossibleTranslations(
            possibleTranslations.map((word) => parseWord(word))
          )
        }
      })
  }
